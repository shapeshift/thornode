...
type ValidatorMgrV10TestSuite struct{}

var _ = Suite(&ValidatorMgrV10TestSuite{})

func (vts *ValidatorMgrV10TestSuite) SetUpSuite(c *C) {
	SetupConfigForTest()
}

func (vts *ValidatorMgrV10TestSuite) TestSetupValidatorNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	mgr := NewDummyMgr()
	vMgr := newValidatorMgrV10(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	vMgr1 := newValidatorMgrV10(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	vMgr2 := newValidatorMgrV10(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV10TestSuite) TestRagnarokForChaosnet(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewManagers(k)
...
func (vts *ValidatorMgrV10TestSuite) TestLowerVersion(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (vts *ValidatorMgrV10TestSuite) TestBadActors(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (vts *ValidatorMgrV10TestSuite) TestRagnarokBond(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	ver := constants.SWVersion
...
func (vts *ValidatorMgrV10TestSuite) TestFindCounToRemove(c *C) {
	// remove one
...
func (vts *ValidatorMgrV10TestSuite) TestFindMaxAbleToLeave(c *C) {
