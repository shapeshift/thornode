...
type SlashingV44Suite struct{}

var _ = Suite(&SlashingV44Suite{})

func (s *SlashingV44Suite) SetUpSuite(c *C) {
	SetupConfigForTest()
}
...
func (s *SlashingV44Suite) TestObservingSlashing(c *C) {
	...
	slasher := newSlasherV44(k, NewDummyEventMgr())
...
func (s *SlashingV44Suite) TestLackObservingErrors(c *C) {
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := newSlasherV44(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV44Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
...
func (s *SlashingV44Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV44Suite) TestDoubleSign(c *C) {
	ctx, _ := setupKeeperForTest(c)
