		...
		for i, txOutItem := range txOut.TxArray {
			...
			if !matchCoin && txOutItem.Coin.Asset.Equals(txOutItem.Chain.GetGasAsset()) {
				...
				if intendToSpend.Equal(actualSpend) {
					...
					// For outbound tx on BTC chain, when THORNode issue the tx out item , it can only estimate the MaxGas that bifrost can spend to send out the tx, because THORNode doesn't
					// know how big the final transaction generated by BTC chain client will be, as such the MasGas is just a top limit.
					// Sometimes the MaxGas is more than it need , or even over limit, for example , bitcoind don't allow to spend more than 0.1 BTC / kb on fee
					// As such bifrost are allow to spend less on Gas , and give the extra on Gas to customer
					// when that happen , here need to artificially increase the Gas , so reserve will subsidise the pool accordingly , because the MaxGas has been deduct from the pool previously
