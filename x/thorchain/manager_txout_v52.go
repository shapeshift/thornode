...
// newTxOutStorageV52 will create a new instance of TxOutStore.
func newTxOutStorageV52(keeper keeper.Keeper, constAccessor constants.ConstantValues, eventMgr EventManager, gasManager GasManager) *TxOutStorageV52 {
	return &TxOutStorageV52{
		keeper:        keeper,
		eventMgr:      eventMgr,
...
func (tos *TxOutStorageV52) EndBlock(ctx cosmos.Context, mgr Manager) error { return nil }
...
func (tos *TxOutStorageV52) UnSafeAddTxOutItem(ctx cosmos.Context, mgr Manager, toi TxOutItem) error {
	...
	if toi.Chain.Equals(common.BCHChain) && !toi.ToAddress.IsValidBCHAddress() {
		return fmt.Errorf("invalid BCH address, legacy address(%s) is not allowed", toi.ToAddress)
	}
...
func (tos *TxOutStorageV52) prepareTxOutItem(ctx cosmos.Context, toi TxOutItem) ([]TxOutItem, error) {
	...
	// BCH chain will convert legacy address to new format automatically , thus when observe it back can't be associated with the original inbound
	if toi.Chain.Equals(common.BCHChain) && !toi.ToAddress.IsValidBCHAddress() {
		return outputs, fmt.Errorf("invalid BCH address, legacy address(%s) is not allowed", toi.ToAddress)
	}
...
func (tos *TxOutStorageV52) addToBlockOut(ctx cosmos.Context, mgr Manager, toi TxOutItem) error {
...
func (tos *TxOutStorageV52) nativeTxOut(ctx cosmos.Context, mgr Manager, toi TxOutItem) error {
...
func (tos *TxOutStorageV52) collectYggdrasilPools(ctx cosmos.Context, tx ObservedTx, gasAsset common.Asset) (Vaults, error) {
...
func (tos *TxOutStorageV52) deductVaultPendingOutboundBalance(ctx cosmos.Context, vault Vault) (Vault, error) {
...
func (tos *TxOutStorageV52) deductVaultBlockPendingOutbound(vault Vault, block *TxOut) Vault {
