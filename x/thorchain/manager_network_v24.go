...
// NetworkMgrV24 is going to manage the vaults
type NetworkMgrV24 struct {
	k          keeper.Keeper
	txOutStore TxOutStore
	eventMgr   EventManager
}

// NewNetworkMgrV24 create a new vault manager
func NewNetworkMgrV24(k keeper.Keeper, txOutStore TxOutStore, eventMgr EventManager) *NetworkMgrV24 {
	return &NetworkMgrV24{
		k:          k,
		txOutStore: txOutStore,
		eventMgr:   eventMgr,
	}
...
func (vm *NetworkMgrV24) processGenesisSetup(ctx cosmos.Context) error {
...
func (vm *NetworkMgrV24) EndBlock(ctx cosmos.Context, mgr Manager, constAccessor constants.ConstantValues) error {
	if common.BlockHeight(ctx) == genesisBlockHeight {
		...
		if (common.BlockHeight(ctx)-vault.StatusSince)%migrateInterval == 0 {
			for _, coin := range vault.Coins {
				...
				if coin.Asset.Equals(gasAsset) {
					...
					gasAmount := gas.Amount.MulUint64(uint64(vault.CoinLengthByChainV24(coin.Asset.Chain)))
...
func (vm *NetworkMgrV24) TriggerKeygen(ctx cosmos.Context, nas NodeAccounts) error {
...
func (vm *NetworkMgrV24) RotateVault(ctx cosmos.Context, vault Vault) error {
	active, err := vm.k.GetAsgardVaultsByStatus(ctx, ActiveVault)
	if err != nil {
		return err
	}
...
// and ragnaroks them
func (vm *NetworkMgrV24) manageChains(ctx cosmos.Context, mgr Manager, constAccessor constants.ConstantValues) error {
...
// ragnarok'ed
func (vm *NetworkMgrV24) findChainsToRetire(ctx cosmos.Context) (common.Chains, error) {
...
func (vm *NetworkMgrV24) RecallChainFunds(ctx cosmos.Context, chain common.Chain, mgr Manager, excludeNodes common.PubKeys) error {
...
func (vm *NetworkMgrV24) ragnarokChain(ctx cosmos.Context, chain common.Chain, nth int64, mgr Manager, constAccessor constants.ConstantValues) error {
	version := vm.k.GetLowestActiveVersion(ctx)
...
func (vm *NetworkMgrV24) UpdateNetwork(ctx cosmos.Context, constAccessor constants.ConstantValues, gasManager GasManager, eventMgr EventManager) error {
...
func (vm *NetworkMgrV24) getTotalProvidedLiquidityRune(ctx cosmos.Context) (Pools, cosmos.Uint, error) {
...
func (vm *NetworkMgrV24) getTotalActiveBond(ctx cosmos.Context) (cosmos.Uint, error) {
...
// Pays out Rewards
func (vm *NetworkMgrV24) payPoolRewards(ctx cosmos.Context, poolRewards []cosmos.Uint, pools Pools) error {
...
func (vm *NetworkMgrV24) calcPoolDeficit(lpDeficit, totalFees, poolFees cosmos.Uint) cosmos.Uint {
...
func (vm *NetworkMgrV24) calcBlockRewards(totalProvidedLiquidity, totalBonded, totalReserve, totalLiquidityFees cosmos.Uint, emissionCurve, incentiveCurve, blocksPerYear int64) (cosmos.Uint, cosmos.Uint, cosmos.Uint) {
...
func (vm *NetworkMgrV24) getPoolShare(incentiveCurve int64, totalProvidedLiquidity, totalBonded, totalRewards cosmos.Uint) cosmos.Uint {
	/*
		Pooled : Share
		0 : 100%
