...
type ValidatorMgrV77TestSuite struct{}

var _ = Suite(&ValidatorMgrV77TestSuite{})

func (vts *ValidatorMgrV77TestSuite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (vts *ValidatorMgrV77TestSuite) TestSetupValidatorNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	mgr := NewDummyMgr()
	vMgr := newValidatorMgrV77(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	vMgr1 := newValidatorMgrV77(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	vMgr2 := newValidatorMgrV77(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV77TestSuite) TestRagnarokForChaosnet(c *C) {
	ctx, mgr := setupManagerForTest(c)
	vMgr := newValidatorMgrV77(mgr.Keeper(), mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV77TestSuite) TestLowerVersion(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (vts *ValidatorMgrV77TestSuite) TestBadActors(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (vts *ValidatorMgrV77TestSuite) TestFindBadActors(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(1000)
...
func (vts *ValidatorMgrV77TestSuite) TestFindLowBondActor(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(1000)
...
func (vts *ValidatorMgrV77TestSuite) TestRagnarokBond(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	ver := GetCurrentVersion()
...
func (vts *ValidatorMgrV77TestSuite) TestGetChangedNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	ver := GetCurrentVersion()
...
func (vts *ValidatorMgrV77TestSuite) TestSplitNext(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgr()
	vMgr := newValidatorMgrV77(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(vMgr, NotNil)
...
func (vts *ValidatorMgrV77TestSuite) TestFindCounToRemove(c *C) {
	// remove one
...
func (vts *ValidatorMgrV77TestSuite) TestFindMaxAbleToLeave(c *C) {
...
func (vts *ValidatorMgrV77TestSuite) TestFindNextVaultNodeAccounts(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgrWithKeeper(k)
	vMgr := newValidatorMgrV77(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(vMgr, NotNil)
...
func (vts *ValidatorMgrV77TestSuite) TestWeightedBondReward(c *C) {
	ctx, k := setupKeeperForTest(c)
