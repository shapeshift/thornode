...
type NetworkManagerV92TestSuite struct{}

var _ = Suite(&NetworkManagerV92TestSuite{})

func (s *NetworkManagerV92TestSuite) SetUpSuite(c *C) {
	SetupConfigForTest()
}

func (s *NetworkManagerV92TestSuite) TestRagnarokChain(c *C) {
	...
	networkMgr := newNetworkMgrV92(keeper, mgr.TxOutStore(), mgr.EventMgr())
	...
	mgr.K = helper
	networkMgr1 := newNetworkMgrV92(helper, mgr1.TxOutStore(), mgr1.EventMgr())
...
func (s *NetworkManagerV92TestSuite) TestUpdateNetwork(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV92(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV92TestSuite) TestCalcBlockRewards(c *C) {
	mgr := NewDummyMgr()
	networkMgr := newNetworkMgrV92(keeper.KVStoreDummy{}, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV92TestSuite) TestCalcPoolDeficit(c *C) {
...
func (*NetworkManagerV92TestSuite) TestProcessGenesisSetup(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
...
func (*NetworkManagerV92TestSuite) TestGetTotalActiveBond(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV92(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV92TestSuite) TestGetTotalLiquidityRune(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV92(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV92TestSuite) TestPayPoolRewards(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV92(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV92TestSuite) TestFindChainsToRetire(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV92(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV92TestSuite) TestRecallChainFunds(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV92(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV92TestSuite) TestRecoverPoolDeficit(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV92(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV92TestSuite) TestRagnarokPool(c *C) {
	...
	networkMgr := newNetworkMgrV92(k, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV92TestSuite) TestCleanupAsgardIndex(c *C) {
	ctx, k := setupKeeperForTest(c)
