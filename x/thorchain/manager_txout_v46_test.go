...
type TxOutStoreV46Suite struct{}

var _ = Suite(&TxOutStoreV46Suite{})

func (s TxOutStoreV46Suite) TestAddGasFees(c *C) {
...
func (s TxOutStoreV46Suite) TestAddOutTxItem(c *C) {
	...
	txOutStore := newTxOutStorageV46(w.keeper, w.mgr.GetConstants(), w.mgr.EventMgr(), w.mgr.GasMgr())
...
func (s TxOutStoreV46Suite) TestAddOutTxItemNotEnoughForFee(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
	vault := GetRandomVault()
	vault.Coins = common.Coins{
		common.NewCoin(common.RuneAsset(), cosmos.NewUint(10000*common.One)),
...
func (s TxOutStoreV46Suite) TestAddOutTxItemWithoutBFT(c *C) {
	w := getHandlerTestWrapper(c, 1, true, true)
	vault := GetRandomVault()
	vault.Coins = common.Coins{
		common.NewCoin(common.BNBAsset, cosmos.NewUint(100*common.One)),
