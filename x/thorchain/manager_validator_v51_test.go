...
type ValidatorMgrV51TestSuite struct{}

var _ = Suite(&ValidatorMgrV51TestSuite{})

func (vts *ValidatorMgrV51TestSuite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (vts *ValidatorMgrV51TestSuite) TestSetupValidatorNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	mgr := NewDummyMgr()
	vMgr := newValidatorMgrV51(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV51TestSuite) TestRagnarokForChaosnet(c *C) {
...
func (vts *ValidatorMgrV51TestSuite) TestLowerVersion(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (vts *ValidatorMgrV51TestSuite) TestBadActors(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (vts *ValidatorMgrV51TestSuite) TestFindBadActors(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ctx = ctx.WithBlockHeight(1000)
...
func (vts *ValidatorMgrV51TestSuite) TestRagnarokBond(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	ver := GetCurrentVersion()
...
func (vts *ValidatorMgrV51TestSuite) TestGetChangedNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	ver := GetCurrentVersion()
...
func (vts *ValidatorMgrV51TestSuite) TestSplitNext(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgr()
	vMgr := newValidatorMgrV51(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(vMgr, NotNil)
...
func (vts *ValidatorMgrV51TestSuite) TestFindCounToRemove(c *C) {
	// remove one
...
func (vts *ValidatorMgrV51TestSuite) TestFindMaxAbleToLeave(c *C) {
...
func (vts *ValidatorMgrV51TestSuite) TestFindNextVaultNodeAccounts(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgrWithKeeper(k)
	vMgr := newValidatorMgrV51(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(vMgr, NotNil)
