...
type SwapSuiteV24 struct{}

var _ = Suite(&SwapSuiteV24{})

func (s *SwapSuiteV24) SetUpSuite(c *C) {
	err := os.Setenv("NET", "other")
	c.Assert(err, IsNil)
	SetupConfigForTest()
}

func (s *SwapSuiteV24) TestSwap(c *C) {
	...
	for _, item := range inputs {
		...
		amount, evts, err := swapV24(ctx, poolStorage, tx, item.target, item.destination, item.tradeTarget, cosmos.NewUint(1000_000), 2, m)
...
func (s *SwapSuiteV24) TestSynthSwap(c *C) {
	...
	// swap rune --> synth
	amount, _, err := swapV24(ctx, k, tx, common.BNBAsset, addr, cosmos.ZeroUint(), cosmos.NewUint(1000_000), 2, m)
	c.Assert(err, IsNil)
	...
	amount, _, err = swapV24(ctx, k, tx, common.RuneAsset(), addr, cosmos.ZeroUint(), cosmos.NewUint(1000_000), 2, m)
	c.Assert(err, IsNil)
