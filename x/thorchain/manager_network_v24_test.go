...
type NetworkManagerV24TestSuite struct{}

var _ = Suite(&NetworkManagerV24TestSuite{})

func (s *NetworkManagerV24TestSuite) SetUpSuite(c *C) {
	SetupConfigForTest()
}

func (s *NetworkManagerV24TestSuite) TestRagnarokChain(c *C) {
	...
	vaultMgr := NewNetworkMgrV24(keeper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV24TestSuite) TestUpdateNetwork(c *C) {
	ctx, k := setupKeeperForTest(c)
	ver := constants.SWVersion
	constAccessor := constants.GetConstantValues(ver)
	helper := NewVaultGenesisSetupTestHelper(k)
	...
	mgr.BeginBlock(ctx)
	vaultMgr := NewNetworkMgrV24(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV24TestSuite) TestCalcBlockRewards(c *C) {
	mgr := NewDummyMgr()
	vaultMgr := NewNetworkMgrV24(keeper.KVStoreDummy{}, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV24TestSuite) TestCalcPoolDeficit(c *C) {
...
func (*NetworkManagerV24TestSuite) TestProcessGenesisSetup(c *C) {
	ctx, k := setupKeeperForTest(c)
	ver := constants.SWVersion
	constAccessor := constants.GetConstantValues(ver)
	helper := NewVaultGenesisSetupTestHelper(k)
...
func (*NetworkManagerV24TestSuite) TestGetTotalActiveBond(c *C) {
	ctx, k := setupKeeperForTest(c)
	helper := NewVaultGenesisSetupTestHelper(k)
	mgr := NewManagers(helper)
	mgr.BeginBlock(ctx)
...
func (*NetworkManagerV24TestSuite) TestGetTotalLiquidityRune(c *C) {
	ctx, k := setupKeeperForTest(c)
	helper := NewVaultGenesisSetupTestHelper(k)
	mgr := NewManagers(helper)
	mgr.BeginBlock(ctx)
...
func (*NetworkManagerV24TestSuite) TestPayPoolRewards(c *C) {
	ctx, k := setupKeeperForTest(c)
	helper := NewVaultGenesisSetupTestHelper(k)
	mgr := NewManagers(helper)
	mgr.BeginBlock(ctx)
...
func (*NetworkManagerV24TestSuite) TestFindChainsToRetire(c *C) {
	ctx, k := setupKeeperForTest(c)
	helper := NewVaultGenesisSetupTestHelper(k)
	mgr := NewManagers(helper)
	mgr.BeginBlock(ctx)
...
func (*NetworkManagerV24TestSuite) TestRecallChainFunds(c *C) {
	ctx, k := setupKeeperForTest(c)
	helper := NewVaultGenesisSetupTestHelper(k)
	mgr := NewManagers(helper)
	mgr.BeginBlock(ctx)
	vaultMgr := NewNetworkMgrV21(helper, mgr.TxOutStore(), mgr.EventMgr())
	helper.failToListActiveAccounts = true
