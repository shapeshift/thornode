...
type ValidatorMgrV5TestSuite struct{}

var _ = Suite(&ValidatorMgrV5TestSuite{})

func (vts *ValidatorMgrV5TestSuite) SetUpSuite(c *C) {
	SetupConfigForTest()
}

func (vts *ValidatorMgrV5TestSuite) TestSetupValidatorNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	mgr := NewDummyMgr()
	vMgr := newValidatorMgrV5(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	vMgr1 := newValidatorMgrV5(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	...
	vMgr2 := newValidatorMgrV5(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
...
func (vts *ValidatorMgrV5TestSuite) TestRagnarokForChaosnet(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewManagers(k)
...
func (vts *ValidatorMgrV5TestSuite) TestLowerVersion(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (vts *ValidatorMgrV5TestSuite) TestBadActors(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (vts *ValidatorMgrV5TestSuite) TestRagnarokBond(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	ver := constants.SWVersion
...
func (vts *ValidatorMgrV5TestSuite) TestFindCounToRemove(c *C) {
	// remove one
...
func (vts *ValidatorMgrV5TestSuite) TestFindMaxAbleToLeave(c *C) {
