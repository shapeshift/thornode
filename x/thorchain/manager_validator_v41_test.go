...
type validatorMgrV41TestSuite struct{}

var _ = Suite(&validatorMgrV41TestSuite{})

func (vts *validatorMgrV41TestSuite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (vts *validatorMgrV41TestSuite) TestSetupValidatorNodes(c *C) {
...
func (vts *validatorMgrV41TestSuite) TestRagnarokForChaosnet(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewManagers(k)
...
func (vts *validatorMgrV41TestSuite) TestLowerVersion(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (vts *validatorMgrV41TestSuite) TestBadActors(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (vts *validatorMgrV41TestSuite) TestFindBadActors(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1000)
...
func (vts *validatorMgrV41TestSuite) TestRagnarokBond(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	ver := GetCurrentVersion()
...
func (vts *validatorMgrV41TestSuite) TestGetChangedNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	ver := GetCurrentVersion()
...
func (vts *validatorMgrV41TestSuite) TestSplitNext(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgr()
	vMgr := newValidatorMgrV1(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(vMgr, NotNil)
...
func (vts *validatorMgrV41TestSuite) TestFindCounToRemove(c *C) {
	// remove one
...
func (vts *validatorMgrV41TestSuite) TestFindMaxAbleToLeave(c *C) {
...
func (vts *validatorMgrV41TestSuite) TestFindNextVaultNodeAccounts(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgr()
	vMgr := newValidatorMgrV1(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(vMgr, NotNil)
