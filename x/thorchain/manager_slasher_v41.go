...
// SlasherV41 is V41 implementation os slasher
type SlasherV41 struct {
	keeper   keeper.Keeper
	eventMgr EventManager
}

// NewSlasherV41 create a new instance of Slasher
func NewSlasherV41(keeper keeper.Keeper, eventMgr EventManager) *SlasherV41 {
	return &SlasherV41{keeper: keeper, eventMgr: eventMgr}
}

// BeginBlock called when a new block get proposed to detect whether there are duplicate vote
func (s *SlasherV41) BeginBlock(ctx cosmos.Context, req abci.RequestBeginBlock, constAccessor constants.ConstantValues) {
...
func (s *SlasherV41) HandleDoubleSign(ctx cosmos.Context, addr crypto.Address, infractionHeight int64, constAccessor constants.ConstantValues) error {
...
func (s *SlasherV41) LackObserving(ctx cosmos.Context, constAccessor constants.ConstantValues) error {
...
func (s *SlasherV41) slashNotObserving(ctx cosmos.Context, txHash common.TxID, constAccessor constants.ConstantValues) error {
...
func (s *SlasherV41) checkSignerAndSlash(ctx cosmos.Context, nodes NodeAccounts, blockHeight int64, signers []cosmos.AccAddress, constAccessor constants.ConstantValues) {
...
func (s *SlasherV41) LackSigning(ctx cosmos.Context, constAccessor constants.ConstantValues, mgr Manager) error {
	var resultErr error
	signingTransPeriod := constAccessor.GetInt64Value(constants.SigningTransactionPeriod)
...
func (s *SlasherV41) SlashVault(ctx cosmos.Context, vaultPK common.PubKey, coins common.Coins, mgr Manager) error {
	if coins.IsEmpty() {
		return nil
	}
	...
	for _, member := range membership {
		...
		for _, coin := range coins {
			...
			if err := s.eventMgr.EmitEvent(ctx, eventSlash); err != nil {
...
func (s *SlasherV41) IncSlashPoints(ctx cosmos.Context, point int64, addresses ...cosmos.AccAddress) {
...
func (s *SlasherV41) DecSlashPoints(ctx cosmos.Context, point int64, addresses ...cosmos.AccAddress) {
	for _, addr := range addresses {
