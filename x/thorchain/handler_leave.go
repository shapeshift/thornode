...
func (h LeaveHandler) Run(ctx cosmos.Context, m cosmos.Msg, version semver.Version, _ constants.ConstantValues) (*cosmos.Result, error) {
	...
	if version.GTE(semver.MustParse("0.5.0")) {
		if err := h.handleV5(ctx, msg, version); err != nil {
			ctx.Logger().Error("fail to process msg leave", "error", err)
			return nil, err
		}
		return &cosmos.Result{}, nil
	}
	if version.GTE(semver.MustParse("0.3.0")) {
		if err := h.handleV3(ctx, msg, version); err != nil {
			ctx.Logger().Error("fail to process msg leave", "error", err)
			return nil, err
		}
		return &cosmos.Result{}, nil
...
func (h LeaveHandler) handle(ctx cosmos.Context, msg MsgLeave, version semver.Version) error {
	...
	// this is required to preserve 0.2.0 logic to avoid consensus failure before the version upgrade
	na := nodeAcc
...
func (h LeaveHandler) handleV3(ctx cosmos.Context, msg MsgLeave, version semver.Version) error {
	nodeAcc, err := h.keeper.GetNodeAccount(ctx, msg.NodeAddress)
	if err != nil {
		return ErrInternal(err, "fail to get node account by bond address")
	}
...
func (h LeaveHandler) handleV5(ctx cosmos.Context, msg MsgLeave, version semver.Version) error {
	nodeAcc, err := h.keeper.GetNodeAccount(ctx, msg.NodeAddress)
	if err != nil {
		return ErrInternal(err, "fail to get node account by bond address")
	}
