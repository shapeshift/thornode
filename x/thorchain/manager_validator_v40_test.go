...
type validatorMgrV40TestSuite struct{}

var _ = Suite(&validatorMgrV40TestSuite{})

func (vts *validatorMgrV40TestSuite) SetUpSuite(_ *C) {
	SetupConfigForTest()
}

func (vts *validatorMgrV40TestSuite) TestSetupValidatorNodes(c *C) {
...
func (vts *validatorMgrV40TestSuite) TestRagnarokForChaosnet(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewManagers(k)
...
func (vts *validatorMgrV40TestSuite) TestLowerVersion(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (vts *validatorMgrV40TestSuite) TestBadActors(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (vts *validatorMgrV40TestSuite) TestFindBadActors(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1000)
...
func (vts *validatorMgrV40TestSuite) TestRagnarokBond(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	ver := GetCurrentVersion()
...
func (vts *validatorMgrV40TestSuite) TestGetChangedNodes(c *C) {
	ctx, k := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(1)
	ver := GetCurrentVersion()
...
func (vts *validatorMgrV40TestSuite) TestSplitNext(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgr()
	vMgr := newValidatorMgrV1(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(vMgr, NotNil)
...
func (vts *validatorMgrV40TestSuite) TestFindCounToRemove(c *C) {
	// remove one
...
func (vts *validatorMgrV40TestSuite) TestFindMaxAbleToLeave(c *C) {
...
func (vts *validatorMgrV40TestSuite) TestFindNextVaultNodeAccounts(c *C) {
	ctx, k := setupKeeperForTest(c)
	mgr := NewDummyMgr()
	vMgr := newValidatorMgrV1(k, mgr.VaultMgr(), mgr.TxOutStore(), mgr.EventMgr())
	c.Assert(vMgr, NotNil)
