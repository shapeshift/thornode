...
// newSlasherV115 create a new instance of Slasher
func newSlasherV115(keeper keeper.Keeper, eventMgr EventManager) *SlasherV115 {
	return &SlasherV115{keeper: keeper, eventMgr: eventMgr}
}
...
func (s *SlasherV115) slashNotObserving(ctx cosmos.Context, txHash common.TxID, constAccessor constants.ConstantValues) error {
...
func (s *SlasherV115) checkSignerAndSlash(ctx cosmos.Context, nodes NodeAccounts, blockHeight int64, signers []cosmos.AccAddress, constAccessor constants.ConstantValues) {
...
func (s *SlasherV115) LackSigning(ctx cosmos.Context, mgr Manager) error {
	...
	for i, tx := range txs.TxArray {
		...
		if tx.OutHash.IsEmpty() {
			...
			if vault.IsYggdrasil() && !mgr.Keeper().IsChainHalted(ctx, tx.Chain) {
...
func (s SlasherV115) slashAndUpdateNodeAccount(ctx cosmos.Context, na types.NodeAccount, coin common.Coin, vault types.Vault, totalBond, totalSlashAmountInRune cosmos.Uint) cosmos.Uint {
...
func (s *SlasherV115) updatePoolFromSlash(ctx cosmos.Context, pool types.Pool, stolenAsset common.Coin, runeCreditAmt cosmos.Uint, mgr Manager) {
...
func (s *SlasherV115) needsNewVault(ctx cosmos.Context, mgr Manager, nas int, signingTransPeriod, startHeight int64, inhash common.TxID, pk common.PubKey) bool {
