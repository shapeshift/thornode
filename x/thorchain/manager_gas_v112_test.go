...
type GasManagerTestSuiteV112 struct{}

var _ = Suite(&GasManagerTestSuiteV112{})

func (GasManagerTestSuiteV112) TestGasManagerV112(c *C) {
	ctx, mgr := setupManagerForTest(c)
	k := mgr.K
	constAccessor := constants.GetConstantValues(GetCurrentVersion())
	gasMgr := newGasMgrV112(constAccessor, k)
...
func (GasManagerTestSuiteV112) TestGetFee(c *C) {
	ctx, mgr := setupManagerForTest(c)
	k := mgr.Keeper()
	constAccessor := constants.GetConstantValues(GetCurrentVersion())
	gasMgr := newGasMgrV112(constAccessor, k)
...
func (GasManagerTestSuiteV112) TestDifferentValidations(c *C) {
	ctx, mgr := setupManagerForTest(c)
	k := mgr.Keeper()
	constAccessor := constants.GetConstantValues(GetCurrentVersion())
	gasMgr := newGasMgrV112(constAccessor, k)
...
func (GasManagerTestSuiteV112) TestGetMaxGas(c *C) {
...
func (GasManagerTestSuiteV112) TestOutboundFeeMultiplier(c *C) {
	ctx, k := setupKeeperForTest(c)
	constAccessor := constants.GetConstantValues(GetCurrentVersion())
	gasMgr := newGasMgrV112(constAccessor, k)
