...
type NetworkManagerV59TestSuite struct{}

var _ = Suite(&NetworkManagerV59TestSuite{})

func (s *NetworkManagerV59TestSuite) SetUpSuite(c *C) {
	SetupConfigForTest()
}

func (s *NetworkManagerV59TestSuite) TestRagnarokChain(c *C) {
	...
	vaultMgr := newNetworkMgrV59(keeper, mgr.TxOutStore(), mgr.EventMgr())
	...
	mgr.K = helper
	vaultMgr1 := newNetworkMgrV59(helper, mgr1.TxOutStore(), mgr1.EventMgr())
...
func (s *NetworkManagerV59TestSuite) TestUpdateNetwork(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	vaultMgr := newNetworkMgrV59(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV59TestSuite) TestCalcBlockRewards(c *C) {
	mgr := NewDummyMgr()
	vaultMgr := newNetworkMgrV59(keeper.KVStoreDummy{}, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV59TestSuite) TestCalcPoolDeficit(c *C) {
...
func (*NetworkManagerV59TestSuite) TestProcessGenesisSetup(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
...
func (*NetworkManagerV59TestSuite) TestGetTotalActiveBond(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	vaultMgr := newNetworkMgrV59(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV59TestSuite) TestGetTotalLiquidityRune(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	vaultMgr := newNetworkMgrV59(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV59TestSuite) TestPayPoolRewards(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	vaultMgr := newNetworkMgrV59(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV59TestSuite) TestFindChainsToRetire(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	vaultMgr := newNetworkMgrV59(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV59TestSuite) TestRecallChainFunds(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	vaultMgr := newNetworkMgrV59(helper, mgr.TxOutStore(), mgr.EventMgr())
