...
// withdrawV63 all the asset
// it returns runeAmt,assetAmount,protectionRuneAmt,units, lastWithdraw,err
func withdrawV63(ctx cosmos.Context, version semver.Version, msg MsgWithdrawLiquidity, manager Manager) (cosmos.Uint, cosmos.Uint, cosmos.Uint, cosmos.Uint, cosmos.Uint, error) {
...
// calcImpLossV63 if there needs to add some imp loss protection, in rune
func calcImpLossV63(lp LiquidityProvider, withdrawBasisPoints cosmos.Uint, protectionBasisPoints int64, pool Pool) (cosmos.Uint, cosmos.Uint, cosmos.Uint) {
	/*
	...
	*/
	...
	A1 := common.GetSafeShare(lp.Units, pool.LPUnits, pool.BalanceAsset)
	R1 := common.GetSafeShare(lp.Units, pool.LPUnits, pool.BalanceRune)
	P1 := R1.Quo(A1)
