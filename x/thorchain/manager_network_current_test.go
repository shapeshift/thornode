...
type NetworkManagerV75TestSuite struct{}

var _ = Suite(&NetworkManagerV75TestSuite{})

func (s *NetworkManagerV75TestSuite) SetUpSuite(c *C) {
	SetupConfigForTest()
}

func (s *NetworkManagerV75TestSuite) TestRagnarokChain(c *C) {
	...
	vaultMgr := newNetworkMgrV75(keeper, mgr.TxOutStore(), mgr.EventMgr())
	...
	mgr.K = helper
	vaultMgr1 := newNetworkMgrV75(helper, mgr1.TxOutStore(), mgr1.EventMgr())
...
func (s *NetworkManagerV75TestSuite) TestUpdateNetwork(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	vaultMgr := newNetworkMgrV75(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV75TestSuite) TestCalcBlockRewards(c *C) {
	mgr := NewDummyMgr()
	vaultMgr := newNetworkMgrV75(keeper.KVStoreDummy{}, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV75TestSuite) TestCalcPoolDeficit(c *C) {
...
func (*NetworkManagerV75TestSuite) TestProcessGenesisSetup(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
...
func (*NetworkManagerV75TestSuite) TestGetTotalActiveBond(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	vaultMgr := newNetworkMgrV75(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV75TestSuite) TestGetTotalLiquidityRune(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	vaultMgr := newNetworkMgrV75(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV75TestSuite) TestPayPoolRewards(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	vaultMgr := newNetworkMgrV75(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV75TestSuite) TestFindChainsToRetire(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	vaultMgr := newNetworkMgrV75(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV75TestSuite) TestRecallChainFunds(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	vaultMgr := newNetworkMgrV75(helper, mgr.TxOutStore(), mgr.EventMgr())
