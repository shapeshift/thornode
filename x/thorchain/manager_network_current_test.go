...
type NetworkManagerV111TestSuite struct{}

var _ = Suite(&NetworkManagerV111TestSuite{})

func (s *NetworkManagerV111TestSuite) SetUpSuite(c *C) {
	SetupConfigForTest()
}

func (s *NetworkManagerV111TestSuite) TestRagnarokChain(c *C) {
	...
	networkMgr := newNetworkMgrV111(keeper, mgr.TxOutStore(), mgr.EventMgr())
	...
	mgr.K = helper
	networkMgr1 := newNetworkMgrV111(helper, mgr1.TxOutStore(), mgr1.EventMgr())
...
func (s *NetworkManagerV111TestSuite) TestUpdateNetwork(c *C) {
	ctx, mgr := setupManagerForTest(c)
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV111(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV111TestSuite) TestCalcBlockRewards(c *C) {
	mgr := NewDummyMgr()
	networkMgr := newNetworkMgrV111(keeper.KVStoreDummy{}, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV111TestSuite) TestCalcPoolDeficit(c *C) {
...
func (*NetworkManagerV111TestSuite) TestProcessGenesisSetup(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
...
func (*NetworkManagerV111TestSuite) TestGetTotalActiveBond(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV111(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV111TestSuite) TestGetTotalLiquidityRune(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV111(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV111TestSuite) TestPayPoolRewards(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV111(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV111TestSuite) TestFindChainsToRetire(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV111(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (*NetworkManagerV111TestSuite) TestRecallChainFunds(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV111(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV111TestSuite) TestRecoverPoolDeficit(c *C) {
	ctx, mgr := setupManagerForTest(c)
	helper := NewVaultGenesisSetupTestHelper(mgr.Keeper())
	mgr.K = helper
	networkMgr := newNetworkMgrV111(helper, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV111TestSuite) TestSaverYieldFunc(c *C) {
	var err error
	ctx, mgr := setupManagerForTest(c)
	net := newNetworkMgrV111(mgr.Keeper(), mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV111TestSuite) TestSaverYieldCall(c *C) {
	...
	networkMgr := newNetworkMgrV111(mgr.Keeper(), mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV111TestSuite) TestRagnarokPool(c *C) {
	...
	networkMgr := newNetworkMgrV111(k, mgr.TxOutStore(), mgr.EventMgr())
...
func (s *NetworkManagerV111TestSuite) TestCleanupAsgardIndex(c *C) {
	ctx, k := setupKeeperForTest(c)
...
func (*NetworkManagerV111TestSuite) TestPOLLiquidityAdd(c *C) {
	ctx, mgr := setupManagerForTest(c)

	net := newNetworkMgrV111(mgr.Keeper(), NewTxStoreDummy(), NewDummyEventMgr())
...
func (*NetworkManagerV111TestSuite) TestPOLLiquidityWithdraw(c *C) {
	ctx, mgr := setupManagerForTest(c)
...
func (*NetworkManagerV111TestSuite) TestFairMergePOLCycle(c *C) {
	ctx, mgr := setupManagerForTest(c)
	net := newNetworkMgrV111(mgr.Keeper(), NewTxStoreDummy(), NewDummyEventMgr())
...
func (s *NetworkManagerV111TestSuite) TestSpawnDerivedAssets(c *C) {
	ctx, mgr := setupManagerForTest(c)

	nmgr := newNetworkMgrV111(mgr.Keeper(), NewTxStoreDummy(), NewDummyEventMgr())
...
func (s *NetworkManagerV111TestSuite) TestSpawnDerivedAssetsBasisPoints(c *C) {
	ctx, mgr := setupManagerForTest(c)
