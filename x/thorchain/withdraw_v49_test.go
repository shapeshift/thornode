...
func (WithdrawSuiteV49) TestWithdraw(c *C) {
	...
	for _, tc := range testCases {
		...
		r, asset, _, _, _, err := withdrawV49(ctx, version, tc.ps, tc.msg, mgr)
...
func (WithdrawSuiteV49) TestWithdrawAsym(c *C) {
	...
	for _, tc := range testCases {
		...
		r, asset, _, _, _, err := withdrawV49(ctx, version, ps, tc.msg, mgr)
		if tc.expectedError != nil {
			c.Assert(err, NotNil)
			c.Check(err.Error(), Equals, tc.expectedError.Error())
			c.Check(r.Uint64(), Equals, tc.runeAmount.Uint64())
...
func (WithdrawSuiteV49) TestWithdrawPendingRuneOrAsset(c *C) {
	...
	runeAmt, assetAmt, _, unitsLeft, gas, err := withdrawV49(ctx, version, k, msg, mgr)
	c.Assert(err, IsNil)
	...
	runeAmt, assetAmt, _, unitsLeft, gas, err = withdrawV49(ctx, version, k, msg1, mgr)
	c.Assert(err, IsNil)
