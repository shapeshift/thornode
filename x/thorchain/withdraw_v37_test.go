...
type WithdrawV37Suite struct{}

var _ = Suite(&WithdrawV37Suite{})

func (s *WithdrawV37Suite) SetUpSuite(c *C) {
	SetupConfigForTest()
}
...
func (s WithdrawV37Suite) TestValidateWithdraw(c *C) {
	...
	for _, item := range inputs {
		...
		err := validateWithdrawV37(ctx, ps, item.msg)
...
func (s WithdrawV37Suite) TestCalculateUnsake(c *C) {
	inputs := []struct {
		name                  string
	...
	for _, item := range inputs {
		c.Logf("name:%s", item.name)
		withDrawRune, withDrawAsset, unitAfter, err := calculateWithdrawV37(item.poolUnit, item.poolRune, item.poolAsset, item.lpUnit, item.percentage, common.EmptyAsset)
...
func (WithdrawV37Suite) TestWithdraw(c *C) {
	...
	for _, tc := range testCases {
		...
		r, asset, _, _, _, err := withdrawV37(ctx, version, tc.ps, tc.msg, mgr)
...
func (WithdrawV37Suite) TestWithdrawAsym(c *C) {
	accountAddr := GetRandomNodeAccount(NodeWhiteListed).NodeAddress
	runeAddress := GetRandomRUNEAddress()
	...
	for _, tc := range testCases {
		...
		r, asset, _, _, _, err := withdrawV37(ctx, version, ps, tc.msg, mgr)
		if tc.expectedError != nil {
			c.Assert(err, NotNil)
			c.Check(err.Error(), Equals, tc.expectedError.Error())
			c.Check(r.Uint64(), Equals, tc.runeAmount.Uint64())
...
func (WithdrawV37Suite) TestWithdrawPendingRuneOrAsset(c *C) {
	version := GetCurrentVersion()
	...
	runeAmt, assetAmt, _, unitsLeft, gas, err := withdrawV37(ctx, version, k, msg, mgr)
	c.Assert(err, IsNil)
	...
	runeAmt, assetAmt, _, unitsLeft, gas, err = withdrawV37(ctx, version, k, msg1, mgr)
	c.Assert(err, IsNil)
