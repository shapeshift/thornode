...
func validateWithdrawV37(ctx cosmos.Context, keeper keeper.Keeper, msg MsgWithdrawLiquidity) error {
...
func withdrawV37(ctx cosmos.Context, version semver.Version, keeper keeper.Keeper, msg MsgWithdrawLiquidity, manager Manager) (cosmos.Uint, cosmos.Uint, cosmos.Uint, cosmos.Uint, cosmos.Uint, error) {
	if err := validateWithdrawV37(ctx, keeper, msg); err != nil {
	...
	if fullProtectionLine > 0 { // if protection line is zero, no imp loss protection is given
		protectionBasisPoints := calcImpLossProtectionAmtV37(ctx, lp.LastAddHeight, fullProtectionLine)
		protectionRuneAmount = calcImpLossV37(lp, msg.BasisPoints, protectionBasisPoints, pool)
	...
	withdrawRune, withDrawAsset, unitAfter, err := calculateWithdrawV37(poolUnits, poolRune, poolAsset, fLiquidityProviderUnit, msg.BasisPoints, assetToWithdraw)
	if err != nil {
...
func calcImpLossV37(lp LiquidityProvider, withdrawBasisPoints cosmos.Uint, protectionBasisPoints int64, pool Pool) cosmos.Uint {
	/*
...
func calcImpLossProtectionAmtV37(ctx cosmos.Context, lastDepositHeight, target int64) int64 {
...
func calculateWithdrawV37(poolUnits, poolRune, poolAsset, lpUnits, withdrawBasisPoints cosmos.Uint, withdrawalAsset common.Asset) (cosmos.Uint, cosmos.Uint, cosmos.Uint, error) {
	if poolUnits.IsZero() {
	...
	if withdrawalAsset.IsRune() {
		return calcAsymWithdrawalV37(unitsToClaim, poolUnits, poolRune), cosmos.ZeroUint(), unitAfter, nil
	}
	return cosmos.ZeroUint(), calcAsymWithdrawalV37(unitsToClaim, poolUnits, poolAsset), unitAfter, nil
}

func calcAsymWithdrawalV37(s, T, A cosmos.Uint) cosmos.Uint {
