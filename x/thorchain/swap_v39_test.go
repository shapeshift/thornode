...
type SwapV39Suite struct{}

var _ = Suite(&SwapV39Suite{})

func (s *SwapV39Suite) SetUpSuite(c *C) {
	err := os.Setenv("NET", "other")
	c.Assert(err, IsNil)
	SetupConfigForTest()
}

func (s *SwapV39Suite) TestSwap(c *C) {
	...
	for _, item := range inputs {
		...
		amount, evts, err := NewSwapperV39().swap(ctx, poolStorage, tx, item.target, item.destination, item.tradeTarget, cosmos.NewUint(1000_000), 2, m)
...
func (s *SwapV39Suite) TestSynthSwap(c *C) {
	...
	// swap rune --> synth
	amount, _, err := NewSwapperV39().swap(ctx, k, tx, common.BNBAsset, addr, cosmos.ZeroUint(), cosmos.NewUint(1000_000), 2, m)
	c.Assert(err, IsNil)
	...
	amount, _, err = NewSwapperV39().swap(ctx, k, tx, common.RuneAsset(), addr, cosmos.ZeroUint(), cosmos.NewUint(1000_000), 2, m)
	c.Assert(err, IsNil)
	...
	tx.Chain = common.BNBChain
	amount, _, err = NewSwapperV39().swap(ctx, k, tx1, common.BNBAsset, addr, cosmos.ZeroUint(), cosmos.NewUint(1000_000), 2, m)
	c.Assert(err, IsNil)
	...
	amount, _, err = NewSwapperV39().swap(ctx, k, tx2, common.BTCAsset, addr, cosmos.ZeroUint(), cosmos.NewUint(1000_000_000_000), 2, m)
	c.Assert(err, NotNil)
