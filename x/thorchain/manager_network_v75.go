...
// NetworkMgrV75 is going to manage the vaults
type NetworkMgrV75 struct {
	k          keeper.Keeper
	txOutStore TxOutStore
	eventMgr   EventManager
}

// newNetworkMgrV75 create a new vault manager
func newNetworkMgrV75(k keeper.Keeper, txOutStore TxOutStore, eventMgr EventManager) *NetworkMgrV75 {
	return &NetworkMgrV75{
		k:          k,
		txOutStore: txOutStore,
		eventMgr:   eventMgr,
	}
...
func (vm *NetworkMgrV75) processGenesisSetup(ctx cosmos.Context) error {
...
func (vm *NetworkMgrV75) EndBlock(ctx cosmos.Context, mgr Manager, constAccessor constants.ConstantValues) error {
	if common.BlockHeight(ctx) == genesisBlockHeight {
			...
			for _, coin := range vault.Coins {
				...
				if coin.Asset.Equals(gasAsset) {
					...
					if amt.IsZero() && nth > 5 {
						...
						runeAmt := p.RuneReimbursementForAssetWithdrawal(coin.Amount)
						if !runeAmt.IsZero() {
...
func (vm *NetworkMgrV75) TriggerKeygen(ctx cosmos.Context, nas NodeAccounts) error {
...
func (vm *NetworkMgrV75) RotateVault(ctx cosmos.Context, vault Vault) error {
	active, err := vm.k.GetAsgardVaultsByStatus(ctx, ActiveVault)
	if err != nil {
		return err
	}
...
// and ragnaroks them
func (vm *NetworkMgrV75) manageChains(ctx cosmos.Context, mgr Manager, constAccessor constants.ConstantValues) error {
...
// ragnarok'ed
func (vm *NetworkMgrV75) findChainsToRetire(ctx cosmos.Context) (common.Chains, error) {
...
func (vm *NetworkMgrV75) RecallChainFunds(ctx cosmos.Context, chain common.Chain, mgr Manager, excludeNodes common.PubKeys) error {
...
func (vm *NetworkMgrV75) ragnarokChain(ctx cosmos.Context, chain common.Chain, nth int64, mgr Manager, constAccessor constants.ConstantValues) error {
...
func (vm *NetworkMgrV75) withdrawLiquidity(ctx cosmos.Context, pool Pool, na NodeAccount, nth int64, mgr Manager, constAccessor constants.ConstantValues) error {
...
func (vm *NetworkMgrV75) UpdateNetwork(ctx cosmos.Context, constAccessor constants.ConstantValues, gasManager GasManager, eventMgr EventManager) error {
...
func (vm *NetworkMgrV75) getTotalProvidedLiquidityRune(ctx cosmos.Context) (Pools, cosmos.Uint, error) {
...
func (vm *NetworkMgrV75) getTotalActiveBond(ctx cosmos.Context) (cosmos.Uint, error) {
...
// Pays out Rewards
func (vm *NetworkMgrV75) payPoolRewards(ctx cosmos.Context, poolRewards []cosmos.Uint, pools Pools) error {
...
func (vm *NetworkMgrV75) calcPoolDeficit(lpDeficit, totalFees, poolFees cosmos.Uint) cosmos.Uint {
...
func (vm *NetworkMgrV75) calcBlockRewards(totalProvidedLiquidity, totalBonded, totalReserve, totalLiquidityFees cosmos.Uint, emissionCurve, incentiveCurve, blocksPerYear int64) (cosmos.Uint, cosmos.Uint, cosmos.Uint, cosmos.Uint) {
...
func (vm *NetworkMgrV75) getPoolShare(incentiveCurve int64, totalProvidedLiquidity, totalBonded, totalRewards cosmos.Uint) cosmos.Uint {
	/*
		Pooled : Share
		0 : 100%
