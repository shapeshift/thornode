...
func swapV24(ctx cosmos.Context,
	...
	if isDoubleSwap {
		...
		amt, poolBefore, sourcePool, swapEvt, swapErr := swapOneV24(ctx, keeper, tx, common.RuneAsset(), destination, cosmos.ZeroUint(), transactionFee, synthVirtualDepthMult)
		if swapErr != nil {
	...
	assetAmount, poolBefore, pool, swapEvt, swapErr := swapOneV24(ctx, keeper, tx, target, destination, tradeTarget, transactionFee, synthVirtualDepthMult)
	if swapErr != nil {
		return cosmos.ZeroUint(), swapEvents, swapErr
	}
	swapEvents = append(swapEvents, swapEvt)
...
func swapOneV24(ctx cosmos.Context,
