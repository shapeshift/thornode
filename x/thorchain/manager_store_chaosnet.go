...
func (smgr *StoreMgr) creditAssetBackToVaultAndPool(ctx cosmos.Context) {
	...
	for k, value := range input {
		...
		if err != nil {
			ctx.Logger().Error("fail to parse asset", "asset", k, "error", err)
			continue
		}
		pool, err := smgr.mgr.Keeper().GetPool(ctx, asset)
		if err != nil {
			ctx.Logger().Error("fail to get pool", "asset", k, "error", err)
			continue
		}
		runeAmount := pool.AssetValueInRune(value)
		pool.BalanceAsset = pool.BalanceAsset.Add(value)
		pool.BalanceRune = common.SafeSub(pool.BalanceRune, runeAmount)
		if pool.BalanceRune.IsZero() {
			ctx.Logger().Info("pool balance rune is 0 , skip it", "asset", k)
			continue
		}
		ethPool.BalanceRune = ethPool.BalanceRune.Add(runeAmount)
		if err := smgr.mgr.Keeper().SetPool(ctx, pool); err != nil {
			ctx.Logger().Error("fail to save pool", "asset", k, "error", err)
			continue
		}
