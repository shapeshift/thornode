...
type SlashingV41Suite struct{}

var _ = Suite(&SlashingV41Suite{})

func (s *SlashingV41Suite) SetUpSuite(c *C) {
	SetupConfigForTest()
}

func (s *SlashingV41Suite) TestObservingSlashing(c *C) {
	var err error
	...
	slasher := NewSlasherV41(k, NewDummyEventMgr())
...
func (s *SlashingV41Suite) TestLackObservingErrors(c *C) {
	...
	ver := GetCurrentVersion()
	constAccessor := constants.GetConstantValues(ver)
	slasher := NewSlasherV41(keeper, NewDummyEventMgr())
	err := slasher.LackObserving(ctx, constAccessor)
	c.Assert(err, IsNil)
}

func (s *SlashingV41Suite) TestNodeSignSlashErrors(c *C) {
	testCases := []struct {
		name        string
...
func (s *SlashingV41Suite) TestNotSigningSlash(c *C) {
	ctx, _ := setupKeeperForTest(c)
	ctx = ctx.WithBlockHeight(201) // set blockheight
...
func (s *SlashingV41Suite) TestNewSlasher(c *C) {
	nas := NodeAccounts{
		GetRandomNodeAccount(NodeActive),
		GetRandomNodeAccount(NodeActive),
	}
...
func (s *SlashingV41Suite) TestDoubleSign(c *C) {
	ctx, _ := setupKeeperForTest(c)
...
func (s *SlashingV41Suite) TestIncreaseDecreaseSlashPoints(c *C) {
	ctx, _ := setupKeeperForTest(c)
