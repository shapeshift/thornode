...
type LitecoinBlockMetaAccessorTestSuite struct{}

var _ = Suite(
	&LitecoinBlockMetaAccessorTestSuite{},
)

func (s *LitecoinBlockMetaAccessorTestSuite) TestNewBlockMetaAccessor(c *C) {
...
func (s *LitecoinBlockMetaAccessorTestSuite) TestBlockMetaAccessor(c *C) {
	memStorage := storage.NewMemStorage()
	db, err := leveldb.Open(memStorage, nil)
	c.Assert(err, IsNil)
