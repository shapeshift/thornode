...
func (s *CosmosTestSuite) TestGetAddress(c *C) {
	...
	cc := CosmosClient{
		cfg:           config.ChainConfiguration{ChainID: common.GAIAChain},
...
func (s *CosmosTestSuite) TestSign(c *C) {
	...
	clientConfig := config.ChainConfiguration{ChainID: common.GAIAChain}
	scannerConfig := config.BlockScannerConfiguration{ChainID: common.GAIAChain}
