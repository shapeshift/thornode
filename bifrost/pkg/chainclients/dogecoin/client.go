...
func NewClient(thorKeys *thorclient.Keys, cfg config.BifrostChainConfiguration, server *tssp.TssServer, bridge *thorclient.ThorchainBridge, m *metrics.Metrics) (*Client, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("fail to create dogecoin rpc client: %w", err)
	}
	...
	dogPrivateKey, err := getDOGEPrivateKey(thorPrivateKey)
	if err != nil {
		return nil, fmt.Errorf("fail to convert private key for DOGE: %w", err)
	}
	ksWrapper, err := NewKeySignWrapper(dogPrivateKey, tssKm)
	if err != nil {
...
func (c *Client) GetAccount(pkey common.PubKey, height *big.Int) (common.Account, error) {
	...
	totalAmt, err := dogutil.NewAmount(total)
	if err != nil {
		return acct, fmt.Errorf("fail to convert total amount: %w", err)
	}
	return common.NewAccount(0, 0,
		common.Coins{
			common.NewCoin(common.DOGEAsset, cosmos.NewUint(uint64(totalAmt))),
		}, false), nil
}
...
func (c *Client) getAsgardAddress() ([]common.Address, error) {
	...
	for _, v := range vaults {
		addr, err := v.PubKey.GetAddress(common.DOGEChain)
		if err != nil {
...
// For dogecoin chain client we want to save the utxo we can spend later to sign
...
func (c *Client) updateNetworkInfo() {
	...
	amt, err := dogutil.NewAmount(networkInfo.RelayFee)
	if err != nil {
...
func (c *Client) getTxIn(tx *btcjson.TxRawResult, height int64, isMemPool bool) (types.TxInItem, error) {
	...
	amt := uint64(amount.ToUnit(dogutil.AmountSatoshi))
	...
	return types.TxInItem{
		...
		To:          toAddr,
		Coins: common.Coins{
			common.NewCoin(common.DOGEAsset, cosmos.NewUint(amt)),
		},
		Memo: memo,
		Gas:  gas,
	}, nil
...
func (c *Client) getSender(tx *btcjson.TxRawResult) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("fail to query raw tx from dogd")
	}
...
func (c *Client) getGas(tx *btcjson.TxRawResult) (common.Gas, error) {
	...
	for _, vin := range tx.Vin {
		...
		vinTx, err := c.client.GetRawTransactionVerbose(txHash)
		if err != nil {
			return common.Gas{}, fmt.Errorf("fail to query raw tx from dogecoin node")
		}

		amount, err := dogutil.NewAmount(vinTx.Vout[vin.Vout].Value)
		if err != nil {
			return nil, err
		}
		sumVin += uint64(amount.ToUnit(dogutil.AmountSatoshi))
	}
	var sumVout uint64 = 0
	for _, vout := range tx.Vout {
		amount, err := dogutil.NewAmount(vout.Value)
		if err != nil {
			return nil, err
		}
		sumVout += uint64(amount.ToUnit(dogutil.AmountSatoshi))
	}
	totalGas := sumVin - sumVout
	return common.Gas{
		common.NewCoin(common.DOGEAsset, cosmos.NewUint(totalGas)),
	}, nil
}
...
func (c *Client) registerAddressInWalletAsWatch(pkey common.PubKey) error {
	addr, err := pkey.GetAddress(common.DOGEChain)
	if err != nil {
		return fmt.Errorf("fail to get DOGE address from pubkey(%s): %w", pkey, err)
	}
...
func (c *Client) getBlockRequiredConfirmation(txIn types.TxIn, height int64) (int64, error) {
	totalTxValue := txIn.GetTotalTransactionValue(common.DOGEAsset, c.asgardAddresses)
